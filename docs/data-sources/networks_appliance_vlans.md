...
- `fixed_ip_assignments` (Attributes Map) The DHCP fixed IP assignments on the VLAN, mapped by MAC address to an object containing 'ip' and 'name'. (see [below for nested schema](#nestedatt--list--fixed_ip_assignments))
...
- `ip` (String) The IP address associated with the fixed IP assignment.
- `name` (String) A descriptive name for the IP assignment.
